...
Namespace Pages.Masters.Folder
    Public Class FolderSettingCreate
...
#Region "ボタンイベント"
        ' 一覧へボタン
        Protected Sub btnFolderList_Click(sender As Object, e As EventArgs)
            Response.Redirect("FolderList.aspx")
        End Sub

        ' 登録ボタン
        Protected Sub btnCreateFolderSetting_Click(sender As Object, e As EventArgs)
            lblError.Text = ""
            lblResult.Text = ""
            ...
            Try
                ...
                Dim repoFolder As New FolderRepository(Utils.GetConnectionString())
                ...
                ' 重複チェック（NULL セーフ一致）
                Dim existsOther As Boolean = repoFolder.ExistsFolder(
                    customerSettingId:=customerSettingId,
                    folderType:=folderType,
                    excludeFolderId:=0 ' 新規のため除外なし
                )
                If existsOther Then
                ...
                existsOther = repoFolder.ExistsFolderPath(
                ...
                Dim newId As Long = repoFolder.InsertFolderNullable(
                    customerSettingId:=customerSettingId,
                    folderType:=folderType,
                    folderPath:=folderPath,
...
#Region "OnPreRender"
        Protected Overrides Sub OnPreRender(e As EventArgs)
            ...
            btnCreateFolderSetting.Attributes("data-error-label-id") = lblError.ClientID
        End Sub
#End Region
