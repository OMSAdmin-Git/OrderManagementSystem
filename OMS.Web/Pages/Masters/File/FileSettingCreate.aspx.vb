...
Namespace Pages.Masters.File
    Public Class FileSettingCreate
...
#Region "ボタンイベント"
        ' 一覧へボタン
        Protected Sub btnFileList_Click(sender As Object, e As EventArgs)
            Response.Redirect("FileList.aspx")
        End Sub

        ' 登録ボタン
        Protected Sub btnCreateFileSetting_Click(sender As Object, e As EventArgs)
            lblError.Text = ""
            lblResult.Text = ""
            ...
            Try
                ...
                ' ログイン情報
                Dim loginUserId As String = PageHelpers.GetUserId(Me)
                Dim programId As String = "FileSettingCreate(Insert)"
                ...
                ' 重複チェック（NULL セーフ一致）
                Dim existsOther As Boolean = repoFile.ExistsFile(
                    customerSettingId:=customerSettingId,
                    folderType:=folderType,
                    excludeFileId:=0 ' 新規のため除外なし
                )
                If existsOther Then
                ...
                Dim newId As Long = repoFile.InsertFileNullable(
                    customerSettingId:=customerSettingId,
                    folderType:=folderType,
...
#Region "OnPreRender"
        Protected Overrides Sub OnPreRender(e As EventArgs)
            ...
            btnCreateFileSetting.Attributes("data-error-label-id") = lblError.ClientID
        End Sub
#End Region
