...
Namespace Pages.Masters.File
    Public Class FileSetting
        Inherits System.Web.UI.Page

#Region "定数・フィールド"
        ' id1 = FileId（NUMBER(10,0) → Long）※未指定は 0
        Private ReadOnly Property FileId As Long
            Get
...
#Region "ページ ライフサイクル"
        Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
            If Not IsPostBack Then
                ...
                LoadFileHeader(FileId)
            End If
        End Sub
#End Region

#Region "ファイルマスタデータ"
        Private Sub LoadFileHeader(fileId As Long)
            ...
            Dim repo As New FileRepository(Utils.GetConnectionString())
            Dim dt As DataTable = repo.GetFile(fileId)
        ...
        Private Sub SetHeaderControls(r As DataRow)
            ...
            ddlFormatType.SelectedValue = If(GetStr(r, "FormatType"), "")
            ddlFileType.SelectedValue = If(GetStr(r, "FileType"), "")
            ddlDelimiter.SelectedValue = If(GetStr(r, "Delimiter"), "")
            ddlEnclosure.SelectedValue = If(GetStr(r, "Enclosure"), "")
            ddlHeaderFlag.SelectedValue = If(GetStr(r, "HeaderFlag"), "")
            ddlCharset.SelectedValue = If(GetStr(r, "Charset"), "")
...
#Region "ボタンイベント"
        ' 保存ボタン
        Protected Sub btnSaveFileSetting_Click(sender As Object, e As EventArgs)
            lblError.Text = ""
            lblResult.Text = ""
            ...
            Try
                ...
                ' 入力値取得
                Dim formatType As String = (If(ddlFormatType.SelectedValue, "")).Trim().ToUpperInvariant()
                Dim fileType As String = (If(ddlFileType.SelectedValue, "")).Trim().ToUpperInvariant()
                Dim delimiter As String = (If(ddlDelimiter.SelectedValue, "")).Trim().ToUpperInvariant()
                Dim enclosure As String = (If(ddlEnclosure.SelectedValue, "")).Trim().ToUpperInvariant()
                Dim headerFlag As String = (If(ddlHeaderFlag.SelectedValue, "")).Trim().ToUpperInvariant()
                Dim charset As String = (If(ddlCharset.SelectedValue, "")).Trim().ToUpperInvariant()
                ...
                Dim pgId As String = "FileSetting(Update)"
                ...
                ' UPDATE
                Dim affected As Integer = repo.UpdateFileWithConcurrency(
                    fileId:=FileId,
...
#Region "OnPreRender"
        Protected Overrides Sub OnPreRender(e As EventArgs)
            ...
            btnSaveFileSetting.Attributes("data-error-label-id") = lblError.ClientID
        End Sub
#End Region
