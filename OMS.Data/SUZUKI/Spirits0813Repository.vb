...
Namespace OMS.Data.SUZUKI
    Public Class Spirits0813Repository
...
#End Region
        ''' <summary>
        ''' Spirits0813Row class をDBに追加する
        ''' </summary>
        ''' <param name="conn"></param>
        ''' <param name="tran"></param>
        ''' <param name="row"></param>
        ''' <returns>string: Number:xxxx Errormessage</returns>
        Public Function Insert(conn As OracleConnection, tran As OracleTransaction, row As Spirits0813Row) As String

            Dim records = New List(Of Spirits0813Row)()
            records.Add(row)
        ...
        ''' <summary>
        ''' Spirits0813Row class リストをDBに追加する (元のコード同等呼び出し
        ''' </summary>
        ''' <param name="records"></param>
        Public Sub InsertRange(records As IEnumerable(Of Spirits0813Row))
        ...
        End Sub
        ''' <summary>
        ''' Spirits0813Row record 追加
        ''' </summary>
        ''' <param name="conn"></param>
        ''' <param name="tran"></param>
        ''' <param name="records"></param>
        ''' <returns>string: Number:xxxx Errormessage</returns>
        Public Function InsertRange(conn As OracleConnection, tran As OracleTransaction, records As List(Of Spirits0813Row)) As String
            Dim errorMessage As String = ""
            If records Is Nothing Then Return "Spirits0813Row InsertRange no record data."
            Try
                Dim sb As New StringBuilder()
                sb.AppendLine($"INSERT INTO SUZUKI_SPIRITS_0813 (")
                ...
                Using cmd As New OracleCommand(sb.ToString(), conn)
                    ...
                    For Each r In records
                        ...
                        cmd.Parameters.Add(":p_publication_date", OracleDbType.Date).Value = Utils.SafeNullable(r.PublicationDate)
                        cmd.Parameters.Add(":p_publication_time", OracleDbType.Int16).Value = Utils.SafeNullable(r.PublicationTime)
                        ...
                        cmd.Parameters.Add(":p_customer_item_name", OracleDbType.Varchar2, 30).Value = SafeVarcharLength(r.CustomerItemName, 30)
                        ...
                        cmd.Parameters.Add(":p_capacity", OracleDbType.Int16).Value = Utils.SafeNullable(r.Capacity)
                        ...
                        cmd.Parameters.Add(":p_arrange_manager", OracleDbType.Varchar2, 7).Value = SafeVarcharLength(r.ArrangeManager, 7)
                        cmd.Parameters.Add(":p_purchase_manager", OracleDbType.Varchar2, 7).Value = SafeVarcharLength(r.PurchaseManager, 7)
                        ...
                        cmd.Parameters.Add(":p_supply_process", OracleDbType.Varchar2, 10).Value = SafeVarchar(r.SupplyProcess, 10)
                        ...
                        cmd.Parameters.Add(":p_delivery_date", OracleDbType.Date).Value = Utils.SafeNullable(r.DeliveryDate)
                        cmd.Parameters.Add(":p_delivery_time", OracleDbType.Varchar2, 4).Value = SafeVarchar(r.DeliveryTime, 4)
                        cmd.Parameters.Add(":p_order_qty", OracleDbType.Int64).Value = Utils.SafeNullable(r.OrderQty)
                        cmd.Parameters.Add(":p_imp_file_id", OracleDbType.Int64).Value = Utils.SafeNullable(r.ImpFileId)
                        cmd.Parameters.Add(":p_imp_run_id", OracleDbType.Int64).Value = Utils.SafeNullable(r.ImpRunId)
                        ...
                        cmd.Parameters.Add(":p_created_at", OracleDbType.Date).Value = Utils.SafeNullable(r.CreatedAt)
                        ...
                        cmd.Parameters.Add(":p_updated_at", OracleDbType.Date).Value = Utils.SafeNullable(r.UpdatedAt)
    ...
    Public Class Spirits0813Row
        ...
        Public Property Leadtime As String                              ' 先行日数				VARCHAR2	3 CHAR
        ...
        Public Property OrderQty As Long?                               ' 注文数				NUMBER		8,0
