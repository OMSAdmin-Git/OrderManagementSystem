...
Namespace OMS.Data.SUZUKI
    Public Class Spirits0602Repository
...
#End Region
        ''' <summary>
        ''' Spirits0602Row class をDBに追加する
        ''' </summary>
        ''' <param name="conn"></param>
        ''' <param name="tran"></param>
        ''' <param name="row"></param>
        ''' <returns>string: Number:xxxx Errormessage</returns>
        Public Function Insert(conn As OracleConnection, tran As OracleTransaction, row As Spirits0602Row) As String

            Dim records = New List(Of Spirits0602Row)()
            records.Add(row)
        ...
        ''' <summary>
        ''' Spirits0602Row class リストをDBに追加する (元のコード同等呼び出し
        ''' </summary>
        ''' <param name="records"></param>
        Public Sub InsertRange(records As IEnumerable(Of Spirits0602Row))
        ...
        End Sub
        ''' <summary>
        ''' Spirits0602Row record 追加
        ''' </summary>
        ''' <param name="conn"></param>
        ''' <param name="tran"></param>
        ''' <param name="records"></param>
        ''' <returns>string: Number:xxxx Errormessage</returns>
        Public Function InsertRange(conn As OracleConnection, tran As OracleTransaction, records As List(Of Spirits0602Row)) As String
            Dim errorMessage As String = ""
            If records Is Nothing Then Return "Spirits0602Row InsertRange no record data."
            Try
                Dim sb As New StringBuilder()
                sb.AppendLine($"INSERT INTO SUZUKI_SPIRITS_0602 (")
                ...
                sb.AppendLine("  capacity, customer_order_no, customer_order_no_process_no, delivery_type, supplier_code, ")
                sb.AppendLine("  supplier_factory_code, supplier_shipping_location, delivery_code, delivery_factory_code, arrange_manager, ")
                sb.AppendLine("  purchase_manager, order_notes, reserve, order_data_type, delivery_date_type, ")
                sb.AppendLine("  production_month_type, delivery_date, delivery_time, order_qty, imp_file_id, ")
                ...
                sb.AppendLine(" :p_capacity, :p_customer_order_no, :p_customer_order_no_process_no, :p_delivery_type, :p_supplier_code, ")
                ...
                sb.AppendLine(" :p_purchase_manager, :p_order_notes, :p_reserve, :p_order_data_type, :p_delivery_date_type, ")
                sb.AppendLine(" :p_production_month_type, :p_delivery_date, :p_delivery_time, :p_order_qty, :p_imp_file_id, ")
                ...
                Using cmd As New OracleCommand(sb.ToString(), conn)
                    ...
                    For Each r In records
                        ...
                        cmd.Parameters.Add(":p_customer_order_no_process_no", OracleDbType.Varchar2, 13).Value = SafeVarchar(r.CustomerOrderNoProcessNo, 13)
                        ...
                        cmd.Parameters.Add(":p_order_notes", OracleDbType.Varchar2, 20).Value = SafeVarcharLength(r.OrderNotes, 20)
                        cmd.Parameters.Add(":p_reserve", OracleDbType.Varchar2, 11).Value = SafeVarchar(r.Reserve, 11)
    ...
    Public Class Spirits0602Row
        ...
        Public Property CustomerOrderNoProcessNo As String      ' 発行番号識別-1		VARCHAR2	13 CHAR
        ...
        Public Property Reserve As String                       ' 予備					VARCHAR2	11 CHAR
